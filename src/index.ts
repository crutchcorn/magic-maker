...
export async function compilePages(dirToScan: string) {
    ...
    for (const fileName of files) {
        ...
        const data = (await import(pathToFileURL(resolve(pagesDir, fileName)) as never)) as Page
        if (fileNameNoExt.startsWith("[...") && fileNameNoExt.endsWith("]")) {
            ...
            const pageHTML = ReactDOM.renderToStaticMarkup(React.createElement(data.default, {}, []))
